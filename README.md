...
- **ключи конфига вместо url** → роутер парсит в адресах ключи конфига и генерирует по ключам правильные url → не нужно завязываться в коде на конкретные url (в т.ч. не нужны спец. компоненты для условного рендеринга)
...
Роуты задаются в виде плоского списка. При сравнении всегда `exact === true`, поэтому порядок описания роутов не важен. Если текущему URL соответствует несколько роутов, будет исключение.
